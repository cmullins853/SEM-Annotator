...
class OnlineSHFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Synchronous Spatial Augmentations
        if self.is_train:
...
class PreprocessedSHFDataset(Dataset):
    ...
    """
    def __init__(self, preproc_dir, image_paths, mask_dir, target_size, fixed_length, patch_size=16, is_train=True, bgr=False):
...
def validate(model, loader, device, threshold=0.5, target_size=1024, accelerator=None):
    """
    Quick validation pass.  Returns mean IoU and mean Dice across the loader
    at the given threshold.
    """
    accelerator = accelerator or _IdentityAccelerator(cpu=(device.type == "cpu"))
    model.eval()
    iou_sum, dice_sum, batch_count = 0.0, 0.0, 0.0
    with torch.no_grad():
        for batch in loader:
            ...
            m = compute_metrics(preds_full, full_masks, thresholds=[threshold])
            iou_sum += m[threshold]['iou']
            dice_sum += m[threshold]['dice']
            batch_count += 1.0
    stats = torch.tensor([iou_sum, dice_sum, batch_count], device=device, dtype=torch.float64)
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(stats, op=dist.ReduceOp.SUM)
    denom = max(float(stats[2].item()), 1.0)
    mean_iou = float(stats[0].item() / denom)
    mean_dice = float(stats[1].item() / denom)
...
def run_training_session(args, fixed_length, seed, output_dir, device):
    ...
    if is_ap_texture:
        ...
        train_dataset = PreprocessedSHFDataset(preproc_dir, [all_image_paths[i] for i in train_idx], args.mask_dir, target_size, fixed_length, patch_size, is_train=True, bgr=args.bgr)
        val_dataset   = PreprocessedSHFDataset(preproc_dir, [all_image_paths[i] for i in val_idx],   args.mask_dir, target_size, fixed_length, patch_size, is_train=False, bgr=args.bgr)
        test_dataset  = PreprocessedSHFDataset(preproc_dir, [all_image_paths[i] for i in test_idx],  args.mask_dir, target_size, fixed_length, patch_size, is_train=False, bgr=args.bgr)
    else:
        # Online SHF (Canny, BTH, etc.)
        full_dataset_train = OnlineSHFDataset(args.image_dir, args.mask_dir, target_size, fixed_length, patch_size, is_train=True,  method=args.method, invert=args.inv, bgr=args.bgr)
        full_dataset_val   = OnlineSHFDataset(args.image_dir, args.mask_dir, target_size, fixed_length, patch_size, is_train=False, method=args.method, invert=args.inv, bgr=args.bgr)
        full_dataset_test  = OnlineSHFDataset(args.image_dir, args.mask_dir, target_size, fixed_length, patch_size, is_train=False, method=args.method, invert=args.inv, bgr=args.bgr)
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if accelerator.is_main_process and (epoch + 1) % 5 == 0:
                ...
                if val_ious:
                    ...
                    plt.title("Validation Metrics (t=0.5)")
